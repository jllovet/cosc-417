  ...
  \item What is ${\cal P}({\cal P}(A))$?
        ...
        \[
          \begin{array}{lll}
             ...
             & A_1 = \Bigg\{\Big\{\emptyset \Big\}, \Big\{\{x\}\Big\}, \Big\{\{y\}\Big\}, \Big\{\{x, y\}\Big\}\Bigg\}                                                                                              \\\\ % singletons
             & A_2 = \Bigg\{\Big\{\emptyset, \{x\}\Big\}, \Big\{\emptyset, \{y\}\Big\}, \Big\{\emptyset, \{x, y\}\Big\}, \Big\{\{x\}, \{y\}\Big\}, \Big\{\{x\}, \{x, y\}\Big\}, \Big\{\{y\}, \{x, y\}\Big\}\Bigg\} \\\\ % pairs
             & A_3 = \Bigg\{\Big\{\emptyset, \{x\}, \{y\}\Big\}, \Big\{\emptyset, \{x\}, \{x, y\}\Big\}, \Big\{\emptyset, \{y\}, \{x, y\}\Big\}, \Big\{\{x\}, \{y\}, \{x, y\}\Big\}\Bigg\}                         \\\\ % triples
             & A_4 = {\cal P}(A) = \Bigg\{\Big\{\emptyset, \{x\}, \{y\}, \{x, y\}\Big\}\Bigg\}
          \end{array}
        \]
...
\begin{enumerate}
  ...
  \item $\{w \colon \mbox{ $w$ has $0$ in every odd position}\}$, with $3$ states.
        ...
        In this diagram, I assume that the empty word $\epsilon$ satisfies the requirement that $w$ has $0$ in every odd position.

        \medskip

        \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=2.8cm]

          \node[initial, state, accepting] (0) {$\epsilon$, even};
          \node[state, accepting] (1) [below of=0] {${0_{odd}}$};
          \node[state] (2) [right of=0] {${1_{odd}}$};
...
In case 2, the function $g$ is \emph{onto} (surjective), but it is not \emph{one-to-one} (injective). This means that $g: \mathbb{N} \to A$ covers every element of A, but there are collisions where for $a, b \in \mathbb{N}, g(a) = g(b) \wedge a \ne b$.
...
Generalizing from Sipser's approach here, we can say that for a function $g: \mathbb{N} \to A$, where A is an infinite set, if $g$ is \emph{onto}, we can modify the function in the following way to make it \emph{one-to-one} as well. If the enumeration produces a collision, skip the element that produces the collision and have $g(n)$ be mapped to the next element of the sequence that does not produce a collision. Let $A_{seq}$ represent the intermediate sequence of elements that is used in a construction of the enumeration of set $A$ (analogous to the sequence created by the procedure used by Sipser in the matrix copied above on the way to enumerating $\mathbb{Q}$). If $A_{seq}$ is a sequence of elements, then $n \in \mathbb{N}, g(n) \in A_{seq}$. If $g(n)$ appears in any of the previously mapped elements, then skip elements of $A_{seq}$ until $g(n)$ is a new unique element.
...
\[
  \begin{aligned}
    ...
    f(n+1)      & = g(m), \text{ where $m$ is the least $\in \mathbb{N}$ s.t. } m \not\in A_{1 \to n}
  \end{aligned}
\]
Then $A_{1 \to n} \subseteq A$ is a subset of $A_{seq}$, and $A_{1 \to n}$ consists of a partial enumeration of $A$. For the element $f(n+1)$, if $f(n+1) \in A_{1 \to n}$, and $f(n+1)$ skips to the least subsequent element $a \in A_{seq}$ such that $a \not\in A_{1 \to n}$.
By performing this modification, we will filter out the collisions, resulting in the function $f: \mathbb{N} \to A$ being \emph{one-to-one} and consequently bijective.
...
Case 1: $A$ and $B$ are disjoint

Assuming that the sets $A$ and $B$ are disjoint, this can be expressed explicitly as the following function, which is a bijection $h: \mathbb{N} \to A \cup B$
...
Case 2: $A$ and $B$ are not disjoint

If the sets $A$ and $B$ are not disjoint, then we can create an enumeration that handles repetitions by using the same procedure as above in Problem 4.
