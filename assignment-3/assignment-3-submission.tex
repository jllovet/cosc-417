...
\begin{center}
  \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=2.5cm]
    ...
    \node[state, accepting] (12) [below of=1] {$\{1,2\}$};
    \node[state] (e) [right of=12] {$\emptyset$};

    \path[->]
    (e) edge [loop right] node[right] {$a,b$} (e)
    (1) edge [bend right] node[left] {$a$} (12)
    (1) edge [bend left] node[above] {$b$} (2)
    (2) edge [bend left] node[right] {$a$} (e)
    (2) edge [bend left] node[above] {$b$} (1)
    (12) edge [loop left] node[left] {$a,b$} (12)
    ;
  \end{tikzpicture}
\end{center}
...
\begin{center}
  \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=2.5cm]
    ...
    \path[->]
    (1) edge [bend left] node[above] {$\epsilon$} (2)
    (1) edge [bend right] node[left] {$a$} (3)
    (2) edge [bend left] node[below] {$a$} (1)
    (3) edge [loop right] node[right] {$b$} (3)
    (3) edge [bend right] node[right] {$a, b$} (2)
    ;
  \end{tikzpicture}
\end{center}
...
\begin{align*}
  B = \Big( & Q = \{\emptyset, \{1\}, \{2\}, \{3\}, \{1,2\}, \{1,3\}, \{2,3\}, \{1,2,3\}\}, \\
            & \Sigma = \{a,b\},                                                             \\
            & \delta \text{ (as defined in the table below)},                               \\
            & q_0 = \{1,2\},                                                                \\
            & F = \{\{2\}, \{1,2\}, \{2,3\}, \{1,2,3\}\} \Big)
\end{align*}
...
\begin{center}
  \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=3cm]
    ...
    \node[initial, state, accepting] (12) [below of=1] {$\{1,2\}$};
    \node[state] (13) [right of=12] {$\{1,3\}$};
    \node[state, accepting] (23) [right of=13] {$\{2,3\}$};
    \node[state, accepting] (123) [right of=23] {$\{1,2,3\}$};

    \path[->]
    (e) edge [loop above] node {$a,b$} (e)
    (1) edge [bend left] node[above] {$a$} (3)
    (1) edge node[above] {$b$} (e)
    (2) edge node[above] {$a$} (12)
    (2) edge [bend right] node[above] {$b$} (e)
    (3) edge node {$a$} (2)
    (3) edge node {$b$} (23)
    (12) edge [bend right=70] node[above] {$a$} (123)
    (12) edge node[left] {$b$} (e)
    (13) edge node {$a,b$} (23)
    (23) edge [bend left] node[below] {$a$} (12)
...
Note: This is incorrect. I can't pick the size of x and z. Will consider further and fix.
...
\begin{enumerate}[I.]
  \item We choose $s = a^{p/2} b^{p/2}$ if $p$ is even.

        If $p$ is odd, we take $s = a^{(p+1)/2} b^{(p+1)/2}$.
  \item The adversary breaks $s = xyz$ so that $y = ab$.
  \item We take $i = 2$. We have won the game: $xyyz$ is not in $L$ because
        it contains the subword $abab$, which is not allowed for strings in $L$.
\end{enumerate}
...
The Pumping Lemma says that for every regular language $A$, there is a number $p$ such that for every string $s \in A$ where $\vert s \vert \ge p$, the string $s$ can be broken up into parts so that $s = xyz$, with the following conditions:
\begin{enumerate}[I.]
  \item for every $i \ge 0$, $xy^iz \in A$
  \item $\vert y \vert > 0$
  \item $\vert xy \vert \le p$
\end{enumerate}

A problem with this putative proof is that the adversary's selection does not satisfy the conditions of the Pumping Lemma for the case where $p = 0$, which is a special case when $p$ is even. In that case, it is not possible for condition II, which says $\vert y \vert > 0$, to be satisfied, because $a^{p/2}b^{p/2} = a^0b^0 = \epsilon$. Hence the cutting of $s = xyz$ so that $y = ab$ is impossible. The value of $y$ in such a case would be $\epsilon$, which has a length $0$, failing the condition of the lemma.

There is a similar issue when $p = 1$. In that case,
\begin{align*}
  s & = a^{(p+1)/2} b^{(p+1)/2} \\
    & = a^{(1+1)/2} b^{(1+1)/2} \\
    & = a^{1} b^{1} \\
...
In this case, condition III is not satisfied. Even if $x = \epsilon$, it is still the case that $\vert xy \vert = \vert ab \vert = 2 > p = 1$. Hence in both cases, when $p$ is even and when it is odd, the Pumping Lemma's requirements cannot be satisfied, which invalidates the putative proof.
